...
def render(
    ...
    learned = patch.name == "learned" and timbre is not None
    if learned:
    ...
    for note in notes:
        ...
        # A learned timbre with a frequency envelope has to be sampled at
        # this note's own harmonics: that is what keeps the formants where
        # they were rather than dragging them along with the pitch.
        note_profile = timbre.amplitudes(max_harmonics, note.f0) if learned else profile

        wave_l = np.zeros(length)
        wave_r = np.zeros(length)
        for h in range(1, max_harmonics + 1):
            amp = note_profile[h - 1] if h - 1 < len(note_profile) else 0.0
            if amp <= 0:
                continue
            f = h * note.f0
            if f >= sr / 2 * 0.92:
