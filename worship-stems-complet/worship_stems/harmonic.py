...
def transcription_available() -> bool:
    return transcription_error() is None


def transcription_error() -> str | None:
    """Import the module that actually runs, not just the package.

    `import basic_pitch` proves nothing: the inference code lives in
    `basic_pitch.inference`, which pulls in `basic_pitch.note_creation`,
    which imports resampy. `pip install basic-pitch --no-deps` - which is
    what the installer does, to keep TensorFlow out - leaves resampy and
    mir-eval behind, so the package imports fine and the first real call
    dies. Import the real thing here so that shows up as a warning at the
    start rather than a crash after separation has already run.
    """
    try:
...
#
# basic-pitch's defaults are tuned for piano-like material: clear onsets,
# notes that decay. A sustained choir note under them does not come back as
# one note - vibrato and the sensitive onset threshold chop it into dozens of
# short ones. Measured against synthesised material with exact ground truth
# (onset 50 ms / pitch 50 cents, offsets ignored), on a four-part SATB chord
# loop of 32 notes:
#
#   0.5 / 0.3 /  60 ms  ->  534 notes,  F1 0.10   (what this used to do)
#   0.8 / 0.6 / 400 ms  ->   32 notes,  F1 0.91
#
# and on a solo vocal line of 14 notes, F1 0.22 -> 1.00. The same ordering
# held with human timing jitter, church reverb and added noise; the gain
# narrows there but never reverses. These are starting points, not truths -
# re-check them against real recordings before trusting them.
#
#   (onset_threshold, frame_threshold, minimum_note_length_ms)
TRANSCRIBE_PROFILES: dict[str, tuple[float, float, float]] = {
    "lead_vocal": (0.8, 0.6, 400.0),
    "bgv": (0.8, 0.6, 400.0),
    "pads": (0.8, 0.6, 400.0),
    "bass": (0.8, 0.6, 400.0),
    "keys": (0.7, 0.5, 250.0),
    "electric_guitar": (0.7, 0.5, 250.0),
}
DEFAULT_PROFILE = (0.7, 0.5, 250.0)
...
def transcribe(
    ...
    min_note_len_ms: float | None = None,
) -> tuple[list[Note], object | None]:
    """Polyphonic note transcription. Returns (notes, pretty_midi object).

    `stem_key` picks the profile above. Any threshold passed explicitly wins
    over the profile, so a caller that knows better still can.
    """
    prof_on, prof_fr, prof_len = TRANSCRIBE_PROFILES.get(stem_key or "", DEFAULT_PROFILE)
    onset_threshold = prof_on if onset_threshold is None else onset_threshold
    frame_threshold = prof_fr if frame_threshold is None else frame_threshold
    min_note_len_ms = prof_len if min_note_len_ms is None else min_note_len_ms
