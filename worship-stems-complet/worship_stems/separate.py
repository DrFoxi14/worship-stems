...
class SeparationEngine:
    ...
    def import_error() -> str | None:
        ...
        return SeparationEngine.decoder_error()

    @staticmethod
    def decoder_error() -> str | None:
        """Check the decoders that only fail once a file is actually read.

        `import audioread` is not a test of anything: audioread loads its
        backends lazily, inside `available_backends()`. Its rawread backend
        imports aifc, sunau and audioop, which Python 3.13 removed from the
        stdlib (PEP 594); audioread 3.1 pulls in the backports for them,
        3.0.x does not and does not say so in its metadata, so pip installs
        3.0.x on 3.13 without complaint. The result is an install that
        passes every check and then dies on the first decode.

        Calling `available_backends()` here is what turns that into an
        error the user sees before waiting, not after.
        """
        try:
            import audioread
        except Exception as exc:  # noqa: BLE001
            return f"audioread is missing ({type(exc).__name__}: {exc})"

        try:
            audioread.available_backends()
        except Exception as exc:  # noqa: BLE001
            missing = getattr(exc, "name", None)
            version = getattr(audioread, "__version__", "unknown")
            if missing in {"aifc", "sunau", "audioop"}:
                return (
                    f"audioread {version} cannot decode on Python "
                    f"{sys.version_info.major}.{sys.version_info.minor}: the stdlib "
                    f"module '{missing}' was removed in 3.13. "
                    f"Fix it with: pip install --upgrade 'audioread>=3.1'"
                )
            hint = f" (missing module: {missing})" if missing else ""
            return f"audioread {version} is broken: {type(exc).__name__}: {exc}{hint}"
        return None
