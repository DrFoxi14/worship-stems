...
# audioread loads its backends lazily, so importing it proves nothing. Its
# rawread backend needs aifc/sunau/audioop, which Python 3.13 removed;
# only audioread >= 3.1 pulls in the backports. Force the load here so a
# broken decoder is caught now rather than mid-separation.
try:
    import audioread
    audioread.available_backends()
    ok("audio decoding (audioread)", True, getattr(audioread, "__version__", ""))
except Exception as exc:
    missing = getattr(exc, "name", None)
    ok("audio decoding (audioread)", False, f"{type(exc).__name__}: {exc}")
    if missing in {"aifc", "sunau", "audioop"}:
        print(f"        '{missing}' was removed from Python {sys.version_info.major}.{sys.version_info.minor}")
    print("        fix:  pip install --upgrade 'audioread>=3.1'")
...
# basic_pitch.inference is the module that actually runs; importing the
# package alone passes even when --no-deps left resampy or mir-eval behind.
try:
    ...
    missing = getattr(exc, "name", None)
    ok("note transcription (MIDI)", False, f"optional - {type(exc).__name__}: {exc}")
    if missing:
