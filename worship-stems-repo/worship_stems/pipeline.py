...
class Pipeline:
    ...
    def _transcribe(
        ...
        if not HARM.transcription_available():
            warnings.append(
                "Note transcription is unavailable. Install it with: "
                "pip install basic-pitch pretty_midi onnxruntime --no-deps"
            )
            return notes_by_stem, midi_by_stem
        ...
        for key in keys:
            if key not in stems:
                continue
            found, midi_obj = HARM.transcribe(stems[key], sr)
            if found:
    ...
    def run(
        ...
        if (self.opt.transcribe_midi or self.opt.note_informed_restore) and leaves:
            if HARM.transcription_available():
                ...
                warnings.append(
                    "Note transcription is unavailable, so the note-informed repair was skipped. "
                    "Install it with: pip install basic-pitch pretty_midi onnxruntime --no-deps"
                )
